...
export class SearchItem {
	...
	createDom(image, name, description, index) {
		...
		this.elements.image.src =
			image || `https://picsum.photos/seed/${Date.now()}/50/50`;
...
export class SearchResults {
	...
	constructor(searchItemsData, searchOverlayContent) {
		console.assert(
			searchItemsData instanceof Array,
			"searchItems be an array",
		);
		...
		this.searchItems.forEach((searchItem) => {
			const shortcutHandlerOpen = (e) => {
				if (
					...
					state.openedItems.push(searchItem);
					state.openedItems = keepUniqueOrdered(state.openedItems);
					const item = aras.IomInnovator.newItem(
						searchItem.data.itemTypeName,
						"get",
					);
					...
					? (e) => {
							if (
								...
								this.searchOverlayContent.elements.input.value =
									"";
								state.openedItems.push(searchItem);
								state.openedItems = keepUniqueOrdered(
									state.openedItems,
								);
								state.setItemTypeName(
									searchItem.data.name,
									searchItem.data.label_plural ||
										searchItem.data.name,
									searchItem.elements.image.src,
								);
							}
						}
					: null;
			...
			if (shortcutHandlerChangeSearch) {
				this.associatedShortcuts["keydown"].push(
					shortcutHandlerChangeSearch,
				);
	...
	getSearchItem(index) {
		console.assert(
			index < this.searchItems.length,
			"Requesting search item out of bounds",
		);
